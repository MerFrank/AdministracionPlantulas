<?php include __DIR__ . '/../db/config.php';
      ...
      <main>
        <div class="container">
          <form id="cotizacionForm" method="post" action="">
            ...
            <div class="mb-3">
              <label for="fechaValidez" class="form-label"
                >Fecha de Validez</label
              >
              <input
                type="date"
                id="fechaValidez"
          ...
          $(document).ready(function () {
            ...
            $("#especie").change(function () {
              ...
              if (especieId) {
                ...
                $variedadSelect
                  .empty()
                  .append('<option value="">Cargando variedades...</option>');
                ...
                $.getJSON("", { especie_id: especieId }, function (data) {
                  if (data.length > 0) {
                    ...
                    $variedadSelect
                      .empty()
                      .append(
                        '<option value="">No hay variedades para esta especie</option>'
                      );
                  }
                }).fail(function () {
                  $variedadSelect
                    .empty()
                    .append(
                      '<option value="">Error al cargar variedades</option>'
                    );
                });
              } else {
            ...
            $("#agregarProducto").click(function () {
              ...
              if (!variedadId || !cantidad || isNaN(cantidad) || !precioUnitario || isNaN(precioUnitario)) {
              ...
              $("#tablaProductos tbody").append(`
                <tr id="producto-${productoId}"
                    data-especie-id="${$("#especie").val()}"
                    data-variedad-id="${$("#variedad").val()}"
            ...
            $("#cotizacionForm").submit(function (e) {
              ...
              const fechaValidez = new Date($("#fechaValidez").val());
              const hoy = new Date();
              hoy.setHours(0, 0, 0, 0);

              if (fechaValidez < hoy) {
                alert("La fecha de validez no puede ser anterior a hoy");
                return;
              }
              ...
              $("#tablaProductos tbody tr").each(function () {
                productos.push({
                  especie_id: $(this).data("especie-id"),
                  variedad_id: $(this).data("variedad-id"),
              ...
              $.ajax({
                url: "cotizacion_procesar.php",
                type: "POST",
                dataType: "json",
                contentType: "application/x-www-form-urlencoded; charset=UTF-8",
                data: {
                  id_clientes: $("#id_clientes").val(),
                  fechaValidez: $("#fechaValidez").val(),
                  notas: $("#notas").val(),
