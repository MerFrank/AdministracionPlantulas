  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $variedad = htmlspecialchars(trim($_POST['variedad']));
    $cantidad = htmlspecialchars(trim($_POST['cantidad']));
    $precio_unitario = htmlspecialchars(trim($_POST['precio_unitario']));
    $fecha_validez = htmlspecialchars(trim($_POST['fecha_validez']));
    // Validar campos requeridos
    if (empty($id_cliente) || empty($especie) || empty($variedad) || empty($cantidad) || empty($precio_unitario) || empty($fecha_validez) ) {
          die("Error: Campos requeridos faltantes");
      }
    $conexion = conn();

    $sql = "INSERT INTO Cotizaciones (id_cliente, especie, variedad, cantidad, precio_unitario, fecha_validez)
        VALUES (?, ?, ?, ?, ?, ?)";
        ...
        <main>
          ...
          <div class="container">
            <form id="cotizacionForm" method="post" action="">
              ...
              <select id="categoria" name="especie" required>
                <option value="">-- Selecciona una Especie --</option>
              </select>

              <label for="producto">Variedad</label>
              <select id="producto" name="variedad" disabled required>
              ...
              <input type="date" id="fechaValidez" name="fecha_validez" required />
